...
const formatThaiDateTime = (dateTimeStr) => {
  ...
  try {
    // ถ้าเป็นรูปแบบ ISO มี Z (UTC) → ตัด Z ทิ้ง
    const cleanStr = dateTimeStr.replace("Z", "");

    // case: "2025-10-05 17:05:00" (ไม่มี Z) → ใช้ตรง ๆ
    const [datePart, timePart] = cleanStr.split("T").length > 1 
      ? cleanStr.split("T") 
      : cleanStr.split(" ");
...
const formatThaiDateTimePrint = (utcDateTimeStr) => {
      ...
      >
        ...
        <Box sx={{
          ...
          <Typography variant="body2" sx={{
            ...
            วันที่พิมพ์: {formatThaiDateTimePrint(currentDateTime.toISOString())}
          </Typography>
        </Box>
    ...
    <Box sx={{
      ...
      <Box sx={{
        ...
        <Box sx={{
          ...
          <InfoItem label="ประเภทการเตรียม" value={`${item.process_name || "-"}`} fontSizes={fontSizes} />
        </Box>
