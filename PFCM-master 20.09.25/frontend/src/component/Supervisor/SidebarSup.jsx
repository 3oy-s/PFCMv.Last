...
const filteredSidebarItems = SIDEBAR_ITEMS
  .map(item => {
    // ⛔️ ซ่อนเมนู "จัดการรถเข็น" ถ้า posId ไม่ใช่ 4 หรือ 6
    if (item.name === "จัดการรถเข็น") {
      if (![4, 6].includes(posId)) {
        return null;
      }
      return item;
    }

    // ⛔️ ซ่อนเมนู "เพิ่ม Line Type/Line name" ถ้า posId ไม่ใช่ 4 หรือ 6
    if (item.name === "เพิ่ม Line Type/Line name") {
      if (![4, 6].includes(posId)) {
        return null;
      }
      return item;
    ...
    return item; // เมนูอื่น ๆ แสดงตามปกติ
  })
